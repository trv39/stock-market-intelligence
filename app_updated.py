...
def rag_query(question: str, ticker: str,
    ...
    valid_idx = [i for i in indices[0] if i >= 0]
    if not valid_idx:
        return "No relevant news found.", pd.DataFrame()
    hits = metadata.iloc[valid_idx].copy()

    if ticker and ticker != "All":
        hits = hits[hits["ticker"] == ticker]
    hits = hits.head(top_k)
    if hits.empty:
        return "No relevant news found for the selected ticker.", hits
...
# ── Page 2: Stock Analysis ───────────────────────────────
    ...
    if df_price.empty:
        ...
        if len(df_price) < 2:
            st.warning("Not enough historical price data.")
            st.stop()
        ...
        df_plot = df_price.tail(period_map[period]).sort_values("date")
